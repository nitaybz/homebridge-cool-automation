...
class CoolAutomationPlatform {
	constructor(log, config, api) {
		...
		this.celsiusHalfSteps = config['celsiusHalfSteps'] || false
	...
	initializeHub(hubConfig, hubNumber) {
		this.log(`Initializing CoolMaster Hub ${hubNumber} with IP: ${hubConfig.ip}`)
		...
		hubConfig.storage.init().then(async () => {
			...
			try {
				...
				this.log.error(`CoolMaster Hub ${hubNumber}: Error fetching devices - ${err}`)
			...
			this.log.error(`CoolMaster Hub ${hubNumber}: Error initializing storage - ${err}`)
		})
	...
	configureAccessory(accessory) {
		...
		if (accessory.hubNumber)
			this.hubs.find(hub => hub.hubNumber === accessory.hubNumber).cachedAccessories.push(accessory)
		else
			this.hubs[0].cachedAccessories.push(accessory)
	}
}
